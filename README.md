...
- **递归子文件夹**: 自动递归扫描所有子文件夹，每个子文件夹在自身目录下生成独立的 `jpg/` 输出目录。
...
- **无污染输出**: 所有结果集中存放在源目录下的 `/jpg` 子目录，源文件保持不变。支持递归处理时，每个子目录分别创建各自的 `jpg/` 输出目录。
- **智能重命名**: 自动处理文件命名冲突。
...
| `--recursive` | `-r` | 递归处理子目录，每个子目录生成独立的 `jpg/` 输出目录 |
...
2. 扫描目录中的所有HEIC、LIVP、PNG、DNG、TIFF和JPG文件（GUI 自动递归子文件夹，CLI 通过 `-r` 选项启用递归）
